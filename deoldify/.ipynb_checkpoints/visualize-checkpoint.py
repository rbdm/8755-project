...
from .filters import IFilter, MasterFilter, ColorizerFilter #, ModelFilter
from .generators import gen_inference_deep, gen_inference_wide, gen_inference_deep_MS
...
class ModelImageVisualizer:
    ...
    def plot_transformed_image(
        ...
        orig.close()
        result_path = self._save_result_image(path, result, results_dir)
        result.close()
        return result_path
    ...
    def _save_result_image(self, source_path: Path, image: Image, results_dir='./results_images') -> Path:
        result_path = str(results_dir) + str(source_path.name)
...
def get_image_colorizer(
    ...
    if model_name is not None:
        return get_stable_image_colorizer(render_factor=render_factor, weights_name = model_name)
    else:
        if artistic:
...
def get_stable_image_colorizer(
    ...
    ########
    denoise: bool = False,
    dn_patch_size: int = 256,
    SR: bool = False,
    sr_patch_size: int = 256,
    enhance: bool = False,
    enhance_patch_size: int = 256,
    ...
    """
    if denoise: filters.insert(0, ModelFilter(model = MIRNet(), weights_path = './pretrained_models/denoising/model_denoising.pth', 
                                            patch_size = dn_patch_size))
    
    if enhance: filters.insert(-1, ModelFilter(model = MIRNet(), weights_path = './pretrained_models/enhancement/model_fivek.pth', 
                                            patch_size = enhance_patch_size))
        
    if SR: filters.append(ModelFilter(model = MIRNet(), weights_path = './pretrained_models/super_resolution/model_SR_x3.pth', 
                                            patch_size = sr_patch_size))
        #filters.insert(-1, ModelFilter(model = MIRNet(), weights_path = './pretrained_models/super_resolution/model_SR_x3.pth', 
        #                                    patch_size = sr_patch_size))
    """
    
    filtr = MasterFilter(filters, render_factor=render_factor)
    vis = ModelImageVisualizer(filtr, results_dir=results_dir)
    return vis
...
def get_artistic_image_colorizer_MS(
    root_folder: Path = Path('./'),
    weights_name: str = 'ColorizeArtistic_gen',
    results_dir='result_images',
    render_factor: int = 35
) -> ModelImageVisualizer:
    learn = gen_inference_deep_MS(root_folder=root_folder, weights_name=weights_name)
    #learn = learn.load(weights_name)
    filtr = MasterFilter([ColorizerFilter(learn=learn)], render_factor=render_factor)
    vis = ModelImageVisualizer(filtr, results_dir=results_dir)
    return vis
